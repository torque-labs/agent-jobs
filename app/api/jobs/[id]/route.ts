...
export async function PATCH(req: Request, ctx: RouteCtx) {
  ...
  try {
    ...
    // Re-register cron from the updated row (handles enable/disable, cron change, etc.)
    try {
      await reloadCronForJob(id);
    } catch (err) {
      console.error(`[api/jobs/${id}] failed to reload cron:`, err);
    }
...
export async function DELETE(_req: Request, ctx: RouteCtx) {
  const { id } = await ctx.params;
  try {
    // Stop any active cron handle first so we don't fire after the row is gone.
    try {
      unregisterCron(id);
    } catch (err) {
      console.error(`[api/jobs/${id}] failed to unregister cron:`, err);
    }
